 ...
 */
{
	...
	"def": {
		...
		"xmqpServiceMarker": {
			"enabled": true,
			"x": 80, "y": -2, "align": "center", "bindToIcon": true,
			"textFormat": {
				"font": "xvm",
				"size": 24
			},
		...
		"hp": {
			...
			"format": "<font alpha='{{alive?{{ready?#FF|#80}}|#80}}'>{{alive?{{hp|{{hp-max}}}}|0/{{hp-max}}}}</font>",
	...
	"playersPanel": {
		...
		"large": {
			...
			// Display format for player nickname (macros allowed, see macros.txt).
			// Формат отображения имени игрока (допускаются макроподстановки, см. macros.txt).
			"nickFormatLeft": "<font face='mono' size='{{xvm-stat?13|0}}' color='{{c:t-winrate}}'>{{t-winrate%2d|--}}%</font>  <font face='mono' size='{{xvm-stat?13|0}}' color='{{c:xr}}' alpha='{{alive?#FF|#80}}'>{{xr|--}}</font>  {{name%.{{xvm-stat?{{r_size>2?10|13}}|15}}s~..}}<font alpha='#A0'>{{clan}}</font>",
			"nickFormatRight": "{{name%.{{xvm-stat?{{r_size>2?10|13}}|15}}s~..}}<font alpha='#A0'>{{clan}}  <font face='mono' size='{{xvm-stat?13|0}}' color='{{c:xr}}' alpha='{{alive?#FF|#80}}'>{{xr|--}}</font>  <font face='mono' size='{{xvm-stat?13|0}}' color='{{c:t-winrate}}'>{{t-winrate%2d|--}}%</font>",
			// Shadow for player name field (default null = no shadow, as in vanillas client).
			// Тень для поля имени игрока (по умолчанию null = без тени, как в чистом клиенте).
			"nickShadowLeft": null,
			"nickShadowRight": null,
